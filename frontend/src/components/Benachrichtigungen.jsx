...
export default function Benachrichtigungen() {
  ...
  return <div className="notifications">
    ...
    {open && <section id="notification-panel" className="notification-panel" aria-label="Benachrichtigungen">
      ...
      {items.map((n)=><article key={n.id}><p><strong>{!n.gelesen && 'Neu: '}</strong>{n.text}</p><small>{new Date(n.erstelltAm).toLocaleString('de-DE')}</small>{!n.gelesen && <button type="button" onClick={()=>read(n.id)}>Als gelesen markieren</button>}</article>)}
    </section>}
  </div>
}
