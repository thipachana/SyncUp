...
@RestController @RequestMapping("/api/me/termine")
public class MeinKalenderController {
    ...
    public MeinKalenderController(TerminRepository terms,TerminService service,SessionSecurity security) {this.terms=terms;this.service=service;this.security=security;}
    ...
    @PostMapping @ResponseStatus(HttpStatus.CREATED) public Termin create(@RequestBody TerminService.TerminRequest data,HttpServletRequest http) {return service.create(data,security.current(http).getBenutzerId(),false);}
